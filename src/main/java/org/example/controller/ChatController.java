...
public class ChatController {
    ...
    public ResponseEntity<ApiResponse<ChatResponse>> chat(@RequestBody ChatRequest request) {
        try {
            ...
            // 获取或创建会话，并准备 ReactAgent 上下文（与普通/流式接口共用）
            ChatService.ChatContext chatContext = chatService.prepareChatContext(request.getId());
            String sessionId = chatContext.session().getSessionId();

            logger.info("开始 ReactAgent 对话（支持自动工具调用）");

            String fullAnswer = chatService.executeChat(chatContext.agent(), request.getQuestion());

            chatService.recordChatTurn(sessionId, request.getQuestion(), fullAnswer);
    ...
    public ResponseEntity<ApiResponse<String>> clearChatHistory(@RequestBody ClearRequest request) {
        try {
            ...
            ChatSessionService.SessionInfo session = chatSessionService.getSession(request.getId());
            if (session != null) {
                chatSessionService.clearSession(request.getId());
    ...
    public SseEmitter chatStream(@RequestBody ChatRequest request) {
        ...
        executor.execute(() -> {
            try {
                ...
                Flux<NodeOutput> stream = chatContext.agent().stream(request.getQuestion());
